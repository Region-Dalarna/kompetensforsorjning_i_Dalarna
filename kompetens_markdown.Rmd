...
Efterfrågan på arbetskraft är i grunden ett teoretiskt begrepp som inte är helt rättfram att mäta. När vi tittar på historisk data över exempelvis antal förvärvsarbetande i olika branscher observerar vi när efterfrågan möter utbud, dvs. när en person blir anställd. Efterfrågan kan dock överstiga utbudet, det blir exempelvis allt vanligare att företag har svårt att hitta arbetskraft, något som inte återspeglas i figurerna nedan. Detta är viktigt att notera.

### 3.1 Förvärvsarbetande

I Dalarna var den klart största branschen `r max(forvarvsarbetande_bransch$år)` , sett till antalet förvärvsarbetande, `r tolower(forvarvsarbetande_bransch %>% group_by(år,region,naringsgren) %>% summarize(antal=sum(antal)) %>% filter(region == "Dalarna",antal== max(antal)) %>% .$naringsgren)` med `r format(forvarvsarbetande_bransch %>% group_by(år,region,naringsgren) %>% summarize(antal=sum(antal)) %>% filter(region == "Dalarna",antal== max(antal)) %>% .$antal,big.mark = " ")` anställda. Andra branscher med många anställda är tillverkning och utvinning och utbildning, med `r format(forvarvsarbetande_bransch %>% group_by(år,region,naringsgren) %>% summarize(antal = sum(antal)) %>% filter(region == "Dalarna",naringsgren == "Tillverkning och utvinning") %>% .$antal,big.mark = " ")` respektive `r format(forvarvsarbetande_bransch %>% group_by(år,region,naringsgren) %>% summarize(antal = sum(antal)) %>% filter(region == "Dalarna",naringsgren == "Utbildning") %>% .$antal,big.mark = " ")` anställda. Störst andel kvinnor återfinns inom `r tolower(forvarvsarbetande_bransch %>% filter(region == "Dalarna",kön == "kvinnor") %>% filter(andel == max(andel)) %>% .$naringsgren)` där ca `r round((forvarvsarbetande_bransch %>% filter(region == "Dalarna",kön == "kvinnor") %>% filter(andel == max(andel)) %>% .$andel)*100,0)` procent av de förvärvsarbetande är kvinnor, medan störst andel män återfinns inom `r tolower(forvarvsarbetande_bransch %>% filter(region == "Dalarna",kön == "män") %>% filter(andel == max(andel)) %>% .$naringsgren)` där ca `r round((forvarvsarbetande_bransch %>% filter(region == "Dalarna",kön == "män") %>% filter(andel == max(andel)) %>% .$andel)*100,0)` procent av de förvärvsarbetande är män.
...
Om vi istället fokuserar på förändringen i antalet förvärvsarbetande inom olika branschgrupper mellan 1990 och `r max(sysselsatta_df_forandring$år)` ser vi en delvis annorlunda bild. Den bransch där antalet förvärvsarbetande har ökat mest är `r tolower(sysselsatta_df_forandring %>% filter(år == max(år)) %>% filter(skillnad == max(.$skillnad)) %>%  .$Näringsgren)` med en ökning på `r format(sysselsatta_df_forandring %>% filter(år == max(år)) %>% filter(skillnad == max(.$skillnad)) %>%  .$skillnad,big.mark= " ")` förvärvsarbetande. Detta beror sannolikt på att många företag idag "outsourcar" tjänster, såsom lokalvård. Om exempelvis SSAB tidigare anställde lokalvårdare direkt så återfanns dessa inom branschen tillverkning och utvinning, men om företaget istället "outsourcar" sådan verksamhet till andra företag räknas lokalvårdarna
sannolikt istället som förvärvsarbetande inom företagstjänster, finans mm.

Den branschgrupp där antalet förvärvsarbetande har minskat mest är `r tolower(sysselsatta_df_forandring %>% filter(år == max(år)) %>% filter(skillnad == min(.$skillnad)) %>%  .$Näringsgren)`, med `r format(abs(sysselsatta_df_forandring %>% filter(år == min(år)) %>% filter(skillnad == min(.$skillnad)) %>%  .$skillnad),big.mark= " ")` färre förvärvsarbetande `r max(sysselsatta_df_forandring$år)` än 1990. Detta beror delvis på det fenomen som diskuterades i föregående avsnitt, men en viktigare förklaring är sannolikt att företag inom tillverkning och utvinning idag är avsevärt mycket mer automatiserade än de var för 30 år sedan. Det behövs helt enkelt färre antal anställda i produktionen.
...
Även mellan branscher varierar kvalifikationskraven av naturliga skäl. Störst andel som arbetar inom yrken av enklare karaktär återfinns i Dalarna inom hotell och restaurang, där mer än var fjärde arbetar inom den typen av yrke. Detta kan delvis förklaras med att hotell och restaurang är den bransch i Dalarna med störst andel utrikes födda. Personer som tillhör denna grupp tenderar att ha förgymnasiala utbildningar i större utsträckning än inrikes födda, se avsnitt 4. Även inom branschgruppen företagstjänster är många yrken av enklare karaktär.  Det omvända gäller inom exempelvis utbildning, där fler än hälften av de anställda arbetar inom yrken som kräver motsvarande högskolekompetens.

```{r, echo=FALSE, fig.width=10, fig.align='Left',fig.cap="Figur 7"}
kompetens_bransch_fig
```

### 3.3 Utbildning och ålder
...
`r tolower(bransch_alder_df_sum %>% filter(alder_grupper == "16-19 år",SNI2007_Grupp_namn != "Okänt") %>%filter(Andel == max(.$Andel)) %>% .$SNI2007_Grupp_namn)` är den bransch där klart största andel yngre arbetar, drygt `r round(bransch_alder_df_sum %>% filter(alder_grupper == "16-19 år") %>%filter(Andel == max(.$Andel)) %>% .$Andel,0)` procent av de förvärvsarbetande är mellan 16 år och 19 år gamla. Även inom kultur, fritid och nöje samt är många av de förvärvsarbetande unga. Detta är ett kvitto på att många unga tar sitt första steg in på arbetsmarknaden inom dessa branscher. Det omvända gäller för jord och skogsbruk, där närmare 3 av 4 av de förvärvsarbetande är äldre än 50 år. Detta kan delvis förklaras med att alla som äger en skogsfastighet räknas som förvärvsarbetande och många av dessa tenderar att vara äldre.

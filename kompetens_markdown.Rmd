...
Sverige och Dalarna har en åldrande befolkning och aktuella prognoser från SCB (<a href="https://www.scb.se/contentassets/3c424ebe4366403da0d30dfba52fba87/uf0515_2023a01_br_am85br2401.pdf/" target="_blank">SCB, 2024</a>) pekar på en utveckling där allt färre i arbetsför ålder ska försörja allt fler unga och gamla. Den demografiska försörjningskvoten mäter förhållandet mellan individer i arbetsför ålder, och unga och gamla. Mellan `r forsorjningskov_min_ar` och `r forsorjningskov_max_ar`  har försörjningskvoten i Dalarna ökat från `r forsorjningskov_min_ar_dalarna_varde` till `r forsorjningskov_max_ar_dalarna_varde`. Det innebär att 100 personer i arbetsför ålder ska försörja ungefär `r round(forsorjningskov_max_ar_dalarna_varde,0)` personer utanför arbetsför ålder. Redan idag är försörjningskvoten i Dalarna den högsta i landet och förväntas öka över tid. Det kommer att förstärka den brist på arbetskraft som råder redan i dag och påverka näringslivets och offentlig sektors möjlighet att rekrytera personal.
...
<br>
I kombination med att försörjningskvoten ökar, minskar även befolkningen i arbetsför ålder över tid. Mellan `r antal_utrikes_inrikes_min_ar` och `r antal_utrikes_inrikes_max_ar` minskade antalet inrikes födda i arbetsför ålder med `r utrikes_forandring_antal` personer. Samtidigt har antalet utrikes födda ökat under samma period, men inte i lika stor omfattning (`r inrikes_forandring_antal`), vilket leder till att befolkningen i arbetsför ålder som helhet krymper. 
...
<br>
Befolkningsutvecklingen i Dalarna har varierat över tid med såväl uppgångar som nedgångar. Den senaste stora nedgången var i samband med den ekonomiska krisen runt millennieskiftet och en uppgång skedde i samband med flyktingkrisen 2016. Såväl 2023 som `r bef_forandring_max_ar` observerades tydliga nedgångar i Dalarnas befolkning. För en mer ingående analys av befolkningsutvecklingen, se <a href="https://region-dalarna.github.io/befolkningsutvecklingen_i_Dalarna/" target="_blank">Befolkningsutvecklingen i Dalarna</a>.
...
Under andra kvartalet `r lediga_jobb_senaste_ar` fanns det cirka `r antal_lediga_jobb` lediga jobb. Ungefär `r andel_privat_lediga_jobb` av dessa fanns inom privat sektor. Efterfrågan inom offentlig sektor har minskat markant på senare år, vilket sannolikt har att göra med att många aktörer inom offentlig sektor har fått försämrad ekonomin och i flera fall infört anställningsstopp.
...
<br>
```{r, echo=FALSE, fig.height=5, fig.width=10, fig.align='center', fig.cap="."}
gg_lediga_jobb$lediga_jobb_Dalarna_E1_2006_2025_kvartal2
```

## Jobbinflöden

Efter nedgången på arbetsmarknaden i samband med finanskrisen 2008, då omsättningen på arbetsmarknaden i termer av jobbinflöde låg på drygt 14 procent, har utvecklingen varit i stort sett positiv fram till pandemin 2020. Då skedde en kraftig, men också tillfällig nedgång. `r jobbinflode_senaste_ar` var jobbinfödet `r jobbinflode_senaste_ar_varde` procent, vilket var något sämre än perioden innan. 
...
mellan Sveriges län. `r enkla_yrken_lan_hogst ` har störst andel som arbetar inom enklare yrken, med ungefär `r enkla_yrken_lan_hogst_andel ` procent av de förvärvsarbetande i länet. Dock jobbar generellt relativt få inom dessa yrken, oavsett region. 

Om vi i stället fokuserar på yrken med kvalifikationskrav som motsvarar gymnasial kompetens är skillnaderna större mellan Sveriges län. Lägst andel återfinns i `r gymnasie_yrken_lan_lagst ` där ungefär `r gymnasie_yrken_lan_lagst_varde` procent av de förvärvsarbetande har den typen av yrken, medan störst andel återfinns i `r gymnasie_yrken_lan_hogst ` med ungefär `r gymnasie_yrken_lan_hogst_varde` procent av de förvärvsarbetande. 

För yrken med högre kvalifikationskrav, exempelvis "motsvarande fördjupad högskolekompetens", är situationen mer eller mindre det omvända. Högst andel har `r hogskola_yrken_lan_hogst` med ungefär `r hogskola_yrken_lan_hogst_varde` procent av de förvärvsarbetande, medan `r hogskola_yrken_lan_lagst ` med ungefär `r hogskola_yrken_lan_lagst_varde` procent har lägst andel. Kvalifikationskraven i Dalarna påminner om de i Kalmar, med relativt stor andel av de förvärvsarbetande inom yrken med motsvarande gymnasial kompetens och färre inom yrken som kräver olika former av högskolekompetens.
...
`r unga_andel_bransch_hogst` är den bransch där den klart största andelen yngre arbetar, `r unga_andel_bransch_hogst_varde` procent av de förvärvsarbetande är mellan 16 och 19 år gamla. Även inom kultur, fritid och nöje är många av de förvärvsarbetande unga, vilket är ett kvitto på att många unga tar sitt första steg in på arbetsmarknaden inom dessa branscher. 

Det omvända gäller för `r tolower(bransch_aldst_andel$bransch)` där cirka `r round(bransch_aldst_andel$andel,0)` procent av de förvärvsarbetande är äldre än 60 år. Detta kan delvis förklaras med att alla som äger en skogsfastighet räknas som förvärvsarbetande och många av dessa tenderar att vara äldre.

<br>
```{r, echo=FALSE, fig.height=5, fig.width=10, fig.align='center', fig.cap="."}
gg_bransch_utb_alder$alder_bransch_andel_Dalarna
```

<br>
Störst antal personer över 60 år arbetar i Dalarna inom `r tolower(bransch_aldst_antal$bransch)`. Ungefär `r format(plyr::round_any(as.numeric(bransch_aldst_antal$antal),100),big.mark = " ")` förvärvsarbetande inom branschen är minst 60 år och kommer således att gå i pension inom 5-10 år. Detta, i kombination med en befolkning som blir allt äldre och därför efterfrågar mer vård, leder till ett stort rekryteringsbehov inom vård och omsorg idag och i framtiden. 
...
Personer som ingår i arbetskraften är antingen förvärvsarbetande eller inskrivna på arbetsförmedlingen. Sedan 2006 har arbetskraftsdeltagandet, för både kvinnor och män i arbetsför ålder, ökat. För män är andelen ungefär `r arbetskraftsdeltagande_senaste_ar_man` procent under `r arbetskraftsdeltagande_senaste_ar` medan kvinnors andel var omkring `r arbetskraftsdeltagande_senaste_ar_kvinna` procent vid samma tidpunkt. Över tid har dock skillnaden mellan män och kvinnors arbetskraftsdeltagande minskat. 
...
<br>
Arbetskraftsdeltagandet i Dalarna under `r arbetskraftsdeltagande_senaste_ar` är högre än riksgenomsnittet för såväl män som kvinnor.
...
I Dalarna är den klart största branschen, sett till antalet förvärvsarbetande, `r tolower(bransch_flest_anstallda %>% pull(bransch))` med ungefär `r format(plyr::round_any(bransch_flest_anstallda %>% pull(Antal),1000),big.mark = " ")` anställda. Detta kan delvis förklaras med att befolkningen i Dalarna, precis som många andra glesbefolkade län, har en relativt hög medelålder, något som medför att mer vård och omsorg efterfrågas. SCB bedömer att behovet av arbetskraft inom vård och omsorg kommer att öka i takt med en åldrande befolkning, särskilt inom äldreomsorgen (<a href="https://www.scb.se/contentassets/3c424ebe4366403da0d30dfba52fba87/uf0515_2023a01_br_am85br2401.pdf/" target="_blank">SCB, 2024</a>).

Andra branscher med många anställda i Dalarna är tillverkning och utvinning samt utbildning, med cirka `r antal_anstallda_tillverkning` respektive `r antal_anstallda_utbildning` anställda. Inom vård och omsorg samt utbildning är en övervägande majoritet av de sysselsatta kvinnor, medan det omvända gäller för tillverkning och utvinning, bygg samt transport.
...
<br>
Historiskt har tillverkning och utvinning varit den största branschen i Dalarna sett till antalet anställda, men vård och omsorg är idag störst.
...
Notera även att branschgrupperingarna i diagrammet nedan skiljer sig något från de i det föregående diagrammet. Exempelvis är branscherna handel och transport sammanslagna baserat på branschgrupperingen 1990.
...
<br>
Bland övriga branscher har det skett en stor ökning av antalet förvärvsarbetande inom företagstjänster och finans där det jobbar ungefär `r bransch_forandring_antal_foretagstjanster` procent fler idag än 1990. Även inom kultur och utbildning jobbar många fler idag än 1990. Ökningen inom företagstjänster beror sannolikt på att många företag idag upphandlar vissa tjänster, såsom rekrytering, lönehantering, lokalvård med mera. Om exempelvis ett tillverkande företag som SSAB tidigare anställde lokalvårdare själva, så återfanns dessa inom branschen tillverkning och utvinning, men om företaget istället upphandlar sådan verksamhet till andra företag specialiserade inom området, räknas lokalvårdarna i stället som förvärvsarbetande inom branschen företagstjänster, finans mm.  
...
<br>
Om den arbetsföra befolkningen delas upp i inrikes respektive utrikes födda, framträder en mer nyanserad bild än den i föregående diagram. Tydligt är att Dalarnas arbetsföra befolkning minskat i antal de senaste decennierna och att detta drivs av att den inrikes födda delen av befolkningen minskar. I Dalarna har antalet inrikes födda invånare i arbetsför ålder minskat från ungefär `r inrikes_antal_min_ar` personer år `r antal_utrikes_inrikes_min_ar` till cirka `r inrikes_antal_max_ar` år  `r antal_utrikes_inrikes_max_ar`. Samtidigt har en relativt stor invandring medfört att utrikes födda invånare i arbetsför ålder har ökat från ungefär `r utrikes_antal_min_ar` personer år `r antal_utrikes_inrikes_min_ar` till cirka `r utrikes_antal_max_ar` år  `r antal_utrikes_inrikes_max_ar`.
...
Det som i första hand påverkar pendlingsmönster är närheten till större städer. Högst andel utpendlare i Sverige har `r utpendling_storst_lan$region`, där `r procent_till_text(utpendling_storst_lan$andel)` boende som förvärvsarbetar gör det i ett annat län, Stockholm i stor utsträckning. Även Södermanland och Västmanland, som båda ligger relativt nära Stockholm, har stor utpendling. Hallands närhet till arbetsmarknaden i Göteborg med omnejd bidrar sannolikt till en utpendlingsandel på cirka `r round(utpendling_halland$andel)` procent. Störst andel inpendlare har `r inpendling_storst_lan$region`. I Dalarna är andelen förvärvsarbetande som pendlar över länsgräns relativt liten.

<br>
```{r, echo=FALSE, fig.height=5, fig.width=9, fig.align='center', fig.cap="."}
gg_pendling_lan$In_och_utpendling_procent
```

<br>
Att flera av kommunerna i Dalarna tillhör större arbetsmarknadsregioner blir tydligt i diagrammet nedan. Störst andel utpendling har `r utpendling_storst_kommun$region` där `r procent_till_text(utpendling_storst_kommun$andel)` av de boende som förvärvsarbetar gör det i en annan kommun, i stor utsträckning Borlänge. Även Gagnef (pendling till Borlänge), Smedjebacken (pendling till Ludvika) och Orsa (pendling till Mora) är kommuner där många arbetspendlar till andra kommuner. Störst inpendling har `r inpendling_storst_kommun$region`, där `r round(inpendling_storst_kommun$andel)` procent av de förvärvsarbetande bor i en annan kommun. I norra Dalarna, med flera kommuner som till ytan är stora, exempelvis Malung-Sälen och Älvdalen, är situationen det omvända. Här bor och arbetar de allra flesta i samma kommun.
...
Utbildningsnivån bland Dalarnas invånare varierar såväl mellan kvinnor och män, som mellan inrikes och utrikes födda. Kvinnor har generellt en högre utbildningsnivå än män. Av alla kvinnor `r alder_txt` i Dalarna har `r kv_eftergymn_andel` procent (inrikes och utrikes födda) en eftergymnasial utbildning jämfört med `r man_eftergymn_andel` procent av männen. De utrikes födda kvinnorna har en något lägre andel eftergymnasialt utbildade än inrikes födda kvinnor men högre andel än de inrikes födda männen. 

Den största skillnaden mellan inrikes och utrikes födda återfinns bland de med lägst utbildningsnivå. Knappt var tionde inrikes född `r alder_txt` har en förgymnasial utbildning, dvs. högst grundskoleutbildning (`r man_inr_forgymn_andel` procent av männen och `r kv_inr_forgymn_andel` procent av kvinnorna). För utrikes födda är det var fjärde som har högst en grundskoleutbildning och här är det inte någon skillnad mellan kvinnor och män. En hög andel lågutbildade är en viktig förklaring till att utrikes födda har en klart lägre sysselsättningsgrad än inrikes födda. Personer med lägre utbildningsnivå har svårare att möta företagens kompetenskrav och hitta jobb, och har därför generellt sett svårare att etablera sig på arbetsmarknaden.
...
<br>
I jämförelse med andra län är utbildningsnivån i Dalarna relativt låg, men också med en relativt stor spridning mellan kvinnor och män. År `r andel_hogutb_sista_ar` hade Dalarna tredje lägst andel med eftergymnasialt utbildade bland Sveriges län. Ungefär `r andel_hogutb_kvinnor_senaste_ar` procent av kvinnorna och `r andel_hogutb_man_senaste_ar` procent av männen i Dalarna har en eftergymnasial utbildning om minst 3 år. Det kan jämföras med `r andel_hogutb_kvinnor_senaste_ar_Stockholm` procent för kvinnor respektive `r andel_hogutb_man_senaste_ar_Stockholm` procent för män i Stockholm, det län som har högst utbildningsnivå i Sverige.

<br>
```{r, echo=FALSE, fig.height=5, fig.width=10, fig.align='center', fig.cap="."}
gg_utbniva_senastear[[names(gg_utbniva_senastear)]]
```

<br>
Utbildningsnivån i samhället har dock ökat avsevärt över tid, vilket vi har berört tidigare. Andelen kvinnor som har minst tre års eftergymnasial utbildning i Dalarna har ökat från ungefär `r andel_hogutb_kvinnor_min_ar` procent till cirka `r andel_hogutb_kvinnor_max_ar` procent mellan `r andel_hogutb_forsta_ar` och `r andel_hogutb_sista_ar`. Andelen män med motsvarande utbildningsnivå har ökat från ungefär `r andel_hogutb_man_min_ar` procent till cirka `r andel_hogutb_man_max_ar` procent under samma tidsperiod. 
...
Vid antagningen hösten `r gymnasieantagning_senaste_ar` antogs flest unga i Dalarna till `r tolower(gymnasieantagning_flest_antagna_totalt)`et, ett program dit enbart personer som inte är behöriga till andra program antas. Bland de nationella programmen antogs flest till `r tolower(glue_collapse(gymnasieantagning_flest_antagna_totalt_ej_intro,sep = ", ", last = " och "))`. Högskoleförberedande program är således mest populära bland länets ungdomar. Bland yrkesförberedande program antogs flest till `r tolower(glue_collapse(gymnasieantagning_flest_antagna_totalt_yrkes,sep = ", ", last = " respektive "))`. 
...
<br>
Det program som ökat klart mest i antalet antagna är introduktionsprogrammet. År `r gymnasieantagning_forsta_ar` antogs `r gymnasieantagning_forsta_ar_intro` personer till programmet, en siffra som ökat till `r gymnasieantagning_senaste_ar_intro` år `r gymnasieantagning_senaste_ar`. Bland studieförberedande program är ökningen störst på ekonomiprogrammet, dit mer än dubbelt så många antas idag som `r min(gymnasieantagning_df$ar)`. Bland program som i första hand kan ses som yrkesförberedande har antalet antagna till naturbruksprogrammet ökat avsevärt sedan `r gymnasieantagning_forsta_ar`, medan antalet antagna till restaurang- och livsmedelsprogrammet har minskat tydligt.
...
<span class = "kommentar">Oklart om detta diagram skall behållas. Väl gammal prognos. Relevant?</span>
...
De som tog examen från Högskolan Dalarna läsåret `r senaste_ar_hogskola` gjorde det i stor utsträckning från två program, `r hogskola_flest_examinerade_program` med `r hogskola_flest_examinerade_antal` examinerade vardera. Efter en nedgång 2015, är antalet examinerade från dessa program idag på ungefär samma nivå som de var runt 2010. 

Ett annat populärt program är företagsekonomi, handel och administration med `r hogskola_examinerade_foretagsekonomi` examinerade. Från detta program examineras fler idag än 2010, men det har varit en viss nedgång sedan 2015. Ett program som minskat avsevärt sedan 2010 är teknik och teknisk industri, där antalet examinerade `r senaste_ar_hogskola` är ungefär en tredjedel av jämfört med 2010.
...
Yrkeshögskolan i Dalarna är i första hand inriktad på utbildningar inom vård och omsorg, vilket återspeglar sig på länets arbetsmarknad. Den vanligaste branschen för examinerade från yrkeshögskolan var år `r folkhogskola_senaste_ar` `r folkhogskola_flest_antagna_program`, med `r folkhogskola_flest_antagna_antal` förvärvsarbetande i Dalarna. Det finns dock även utbildningar inom tillverkning och utvinning, då specifikt utvinning av annan malm. 
...
<br>
Det vanligaste inriktningen för personer som tagit en examen för yrkeshögskolan och förvärvsarbetar i Dalarna var år `r folkhogskola_inriktning_flest_arbetar`, med `r folkhogskola_inriktning_flest_arbetar_antal` förvärvsarbetande. Även relativt många med inriktningar mot psykiatrisk vård och tandsköterska förvärvsarbetar i Dalarna.
...
<br>
2018 påbörjade drygt 190 personer en YH-utbildning i Dalarna. De efterföljande tre åren ökade antalet till cirka 250 för att därefter minska till knappt 160 personer under 2022-2024 (utbudet fastställs över en treårsperiod). Utbudet från 2025-2027 fastställs i nästa ansökningsperiod.

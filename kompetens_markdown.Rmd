...
I Dalarna har de unga (0-19 år) samt de arbetsföra (20-64) minskat sedan 1968, medan de äldre (65-79 år och 80+ år) har ökat under samma tidsperiod, en utveckling som dessutom prognostiseras fortsätta i framtiden. Det gör att regionen kan komma att stå inför ett flertal utmaningar i framtiden, däribland:
...
Den största näringen i Dalarna, där flest arbetar, var under år `r max(forvarvsarbetande_90_senastear$år)` hälso och sjukvård, med ungefär `r plyr::round_any(forvarvsarbetande_90_senastear %>% filter(Näringsgren == "enh för hälso- och sjukvård, socialtjänst; veterinärer",år == max(år)) %>% .$antal,1000)` sysselsatta. Även inom handel, transport, magasinering och kommunikation samt utvinning av mineral och tillverkningsindustri arbetade många i Dalarna `r max(forvarvsarbetande_90_senastear$år)`. I den sistnämnda branschgruppen har dock antalet förvärvsarbetande minskat stadigt sedan 1990-talet. I övrigt har antalet förvärvsarbetande ökat inom bland annat kreditinstitut, fastighetsförvaltning och företagstjänster samt forskning, utveckling och utbildning. I den förstnämnda branschgruppen återfinns bland annat sysselsatta inom IT-sektorn, vilket kan förklara en del av uppgången.

```{r, echo=FALSE, fig.width=10, fig.align='Left',fig.cap="Figur 2"}
gg_forv_90$forvarvsarbetande_90_totalt_Dalarna
```

I Dalarnas största kommun, Falun, jobbar majoriteten inom vård och omsorg, följt av utbildning, företagstjänster, offentlig förvaltning, handel, tillverkning, bygg, kultur, fritid och nöje. 
I Dalarnas näst största kommun, Borlänge, så jobbar majoriteten av befolkningen inom handel, följt av vård och omsorg, offentlig förvaltning, utbildning, tillverkning, företagstjänster, bygg och transport. 
I Ludvika så är det vanligast att jobba inom tillverkningsindustrin, följt av vård och omsorg, utbildning, handel, företagstjänster samt bygg. 
I Avesta så är det även där vanligast att jobba inom tillverkning, följt av vård och omsorg, handel, utbildning, bygg, företagstjänster samt transport. 
I Mora så jobbar de flesta inom vård och omsorg, följt av tillverkning, handel, utbildning, bygg och företagstjänster. 
I Leksand arbetar de flesta inom handel, följt av vård och omsorg, utbildning, tillverkning, bygg och företagstjänster, hotell och restaurang samt jord- och skogsbruk.
I Hedemora arbetar de flesta inom vård och omsorg, följt av tillverkning, utbildning, handel och bygg.
I Säter så är den största branschen vård och omsorg, sen i fallande ordning kommer följande branscher: utbildning, bygg, tillverkning, jord- och skogsbruk, kultur, fritid och nöje, samt handel.
I Rättvik arbetar de flesta inom vård och omsorg, följt av utbildning, bygg, tillverkning, handel, jord- och skogsbruk, företagstjänster samt hotell och restaurang. 
I Smedjebacken arbetar de flesta inom tillverkningsindustrin, följt av vård och omsorg, utbildning, bygg, handel samt företagstjänster.  
I Gagnef är den största branschen där flest arbetar inom är vård och omsorg, följt av tillverkningsekonomi, utbildning, bygg, offentlig förvaltning, samt handel. 
I Malung-Sälen arbetar flest inom vård och omsorg, följt av bygg, kultur, fritid och nöje, utbildning, handel, tillverkning och företagstjänster.  
I Älvdalen så jobbar flertalet inom vård och omsorg, bygg, tillverkning, handel, utbildning och 
I Orsa så jobbar de flesta inom vård och omsorg, följt av bygg, transport, utbildning, kultur och fritid, samt jord- och skogsbruk.
I Dalarnas minsta kommun sett till befolkning, Vansbro, jobbar de flesta inom tillverkningsindustrin, följt av handel, vård och omsorg, utbildning, och bygg.
Det finns fler branscher i varje kommun, vilket kan ses i figur 3.
...
I den vänstra cirkeln finns branscher efter antal anställda bland kvinnor, i den högra finns antal branscher efter anställda män. Kvinnor arbetar framförallt inom vård och omsorg och utbildning. Även handel, offentlig förvaltning, företagstjänster, tillverkning, kultur, samt fritid och nöje i fallande ordning. I den högra cirkeln så visas det att män framförallt jobbar inom tillverkningsindustrin, bygg, handel, företagstjänster, vård och omsorg, transport, utbildning, offentlig förvaltning samt jord- och skogsbruk. 
...
De orangea staplarna är andelen kvinnor som har minst tre års eftergymnasial utbildning i Dalarna. Bland kvinnorna så har andelen med tre års eftergymnasial utbildning ökat från ungefär `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == min(år),kön == "kvinnor") %>% .$total,0)` procent till ca `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == max(år),kön == "kvinnor") %>% .$total,0)` procent mellan `r min(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` till `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)`. De gröna staplarna är andelen män med minst tre år eftergymnasial utbildning i Dalarna. Andelen män med tre års eftergymnasial utbildning har ökat från ungefär `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == min(år),kön == "män") %>% .$total,0)` procent `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == max(år),kön == "kvinnor") %>% .$total,0)` procent mellan `r min(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` till ca `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == max(år),kön == "män") %>% .$total,0)` procent `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)`. Det går att utläsa från tabellen att andel kvinnor med tre års eftergymnasial utbildning är dubbelt så stor `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` mot mäns andel. `r min(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` var något större andel män högutbildade, jämfört med kvinnor.
...
```{r, echo=FALSE, fig.width=10, fig.align='Left',fig.cap="Figur 20", include = FALSE}
...
Ytterligare en viktig aspekt av en regions arbetsmarknad är hur lätt det är för arbetsgivare att hitta arbetskraft. Bland Sveriges län varierar den upplevda bristen på arbetskraft relativt mycket. Lättast att hitta kompetens år `r max(kompetensbrist_df$År)` har småföretag i `r kompetensbrist_df %>% filter(År == max(År)) %>% filter(Andel == min(Andel)) %>% .$Region`, där ungefär `r round(kompetensbrist_df %>% filter(År == max(År)) %>% filter(Andel == min(Andel)) %>% .$Andel,0)` procent av småföretagen uppger att de har brist på kompetens. Att jämföra med `r kompetensbrist_df %>% filter(År == max(År)) %>% filter(Andel == max(Andel)) %>% .$Region`, där ungefär `r round(kompetensbrist_df %>% filter(År == max(År)) %>% filter(Andel == max(Andel)) %>% .$Andel,0)` procent av småföretagen uppger att de har samma brist. I Dalarna upplever ungefär `r round(kompetensbrist_df %>% filter(År == max(År)) %>% filter(Region == "Dalarnas län") %>% .$Andel,0)` procent av småföretagarna att de har svårt att hitta arbetskraft, vilket är en ökning med `r round(kompetensbrist_df %>% filter(År == max(År)) %>% filter(Region == "Dalarnas län") %>% .$Andel - kompetensbrist_df %>% filter(År == "2020") %>% filter(Region == "Dalarnas län") %>% .$Andel ,0)` procentenheter sedan undersökningen som gjordes 2020.

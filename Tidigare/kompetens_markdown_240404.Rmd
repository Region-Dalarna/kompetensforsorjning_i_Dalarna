...
Under `r max(forvarvsarbetande_90_senastear$år)` som den mest dominerande näringen i Dalarna när det gäller sysselsättning, och dess betydelse har dessutom ökat något under 2010-talet. Idag jobbar ungefär `r format(plyr::round_any(forvarvsarbetande_90_senastear %>% filter(Näringsgren == "enh för hälso- och sjukvård, socialtjänst; veterinärer",år == max(år)) %>% .$antal,1000),big.mark=" ")` inom denna branschgrupp. Samtidigt var det inte bara inom hälso- och sjukvården som många invånare i Dalarna fann arbete; handel, transport, magasinering, kommunikation samt utvinning av mineral och tillverkningsindustri var också betydande sysselsättningssektorer under `r max(forvarvsarbetande_90_senastear$år)`. 
...
```{r, echo=FALSE,message = FALSE, warning = FALSE, fig.width=10, fig.align='Center',fig.cap="Figur 4"}
...
I figuren nedan visar de orangea staplarna andelen kvinnor som har minst tre års eftergymnasial utbildning i Dalarna. Bland kvinnorna så har andelen med tre års eftergymnasial utbildning ökat från ungefär `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == min(år),kön == "kvinnor") %>% .$total,0)` procent till ca `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == max(år),kön == "kvinnor") %>% .$total,0)` procent mellan `r min(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` till `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)`. De gröna staplarna är andelen män med minst tre år eftergymnasial utbildning i Dalarna. Andelen män med tre års eftergymnasial utbildning har ökat från ungefär `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == min(år),kön == "män") %>% .$total,0)` procent `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == max(år),kön == "kvinnor") %>% .$total,0)` procent mellan `r min(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` till ca `r round(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data %>% filter(år == max(år),kön == "män") %>% .$total,0)` procent `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)`. Det går att utläsa från tabellen att andel kvinnor med tre års eftergymnasial utbildning är dubbelt så stor `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)` mot mäns andel. Det finns flera möjliga förklaringar till det, dels att kvinnor jobbar i högre utsträckning inom vård och omsorg samt utbildning, där de flesta yrkesgrupper kräver eftergymnasial utbildning på minst tre år. Det finns även andra förklaringar, exempelvis att kvinnor generellt sätt, oavsett utbildning, påbörjar eftergymnasiala studier i högre utsträckning än män (Lunds Universitet, 2017; SCB, 2021; SCB, 2022).

HOPPET 99-00 BEHÖVER DISKUTERAS. HAR FÖR MIG ATT DET HAR NÅGOT ATT GÖRA MED HUR MAN MÄTER /JON

```{r, echo=FALSE, fig.width=10, fig.align='Left',fig.cap="Figur 5"}
gg_utbniva_85$hogutb_andel_ar20_1985_2022
```

Stolparna är andelar med minst tre års eftergymnasial utbildning år `r max(gg_utbniva_85$hogutb_andel_ar20_1985_2022$data$år)`. Dalarna ligger näst längst ned i grafen, vilket visar att Dalarna har näst lägst andel med eftergymnasialt utbildade bland Sveriges län. Ungefär `r round(gg_utbniva_senastear$hogutb_andel_ar_2022$data %>% filter(kön == "kvinnor",region == "Dalarna") %>% .$total,0)` procent av kvinnorna och `r round(gg_utbniva_senastear$hogutb_andel_ar_2022$data %>% filter(kön == "män",region == "Dalarna") %>% .$total,0)` av männen i Dalarna har en eftergymnasial utbildning om minst 3 år, att jämföra med `r round(gg_utbniva_senastear$hogutb_andel_ar_2022$data %>% filter(kön == "kvinnor",region == "Stockholm") %>% .$total,0)` procent för kvinnor och `r round(gg_utbniva_senastear$hogutb_andel_ar_2022$data %>% filter(kön == "män",region == "Stockholm") %>% .$total,0)` i Stockholm, de län som har högst utbildningsnivå i Sverige. Spridningen mellan könen är således klart större i Dalarna än i Stockholm. 
...
I Dalarna är den klart största branschen, sett till antalet förvärvsarbetande, `r tolower(antal_forvarvsarbetande_bransch %>% group_by(år,bransch) %>% summarize(Antal=sum(Antal)) %>% filter(Antal== max(Antal)) %>% .$bransch)` med ungefär `r format(plyr::round_any(antal_forvarvsarbetande_bransch %>% group_by(år,bransch) %>% summarize(Antal = sum(Antal)) %>% filter(Antal== max(Antal)) %>% .$Antal,100),big.mark = " ")` anställda. Detta kan delvis förklaras med att befolkningen i Dalarna, precis som många andra glesbefolkade län, har en relativt hög medelålder, något som medför att mer vård och omsorg efterfrågas. Andra branscher med många anställda i Dalarna är tillverkning och utvinning samt utbildning, med ca `r format(plyr::round_any(antal_forvarvsarbetande_bransch %>% group_by(år,bransch) %>% summarize(Antal = sum(Antal)) %>% filter(bransch == "Tillverkning och utvinning") %>% .$Antal,100),big.mark = " ")` respektive ungefär `r format(plyr::round_any(antal_forvarvsarbetande_bransch %>% group_by(år,bransch) %>% summarize(Antal = sum(Antal)) %>% filter(bransch == "Utbildning") %>% .$Antal,100),big.mark = " ")` anställda. Inom vård och omsorg samt utbildning är en övervägande majoritet av de sysselsatta kvinnor, medan det omvända gäller för bygg och transport.

```{r, echo=FALSE, fig.width=10, fig.align='Left',fig.cap="Figur 8"}
gg_forv_senastear$antal_per_bransch
```

Hur många som jobbar inom olika branscher i Dalarna har förändrats genom åren. En tydlig utveckling, mellan 1990 och `r max(forvarvsarbetande_90_forandring$år)` är att många fler arbetar i företag som sysslar med `r tolower(forvarvsarbetande_90_forandring %>% filter(år == max(år)) %>% filter(skillnad == max(.$skillnad)) %>%  .$Näringsgren)`, en ökning på ungefär `r format(plyr::round_any(forvarvsarbetande_90_forandring %>% filter(år == max(år)) %>% filter(skillnad == max(.$skillnad)) %>%  .$skillnad,100),big.mark= " ")` förvärvsarbetande. Detta beror sannolikt på att många företag idag lejer bort vissa tjänster, såsom lokalvård. Om exempelvis ett tillverkande företag som SSAB tidigare anställde lokalvårdare själva, så återfanns dessa inom branschen tillverkning och utvinning, men om företaget istället lejer ut sådan verksamhet till andra företag specialiserade inom området, räknas lokalvårdarna istället som förvärvsarbetande inom företagstjänster, finans mm.

Den bransch i Dalarna där antalet förvärvsarbetande har minskat mest är `r tolower(forvarvsarbetande_90_forandring %>% filter(år == max(år)) %>% filter(skillnad == min(.$skillnad)) %>%  .$Näringsgren)`, med ca `r format(plyr::round_any(abs(forvarvsarbetande_90_forandring %>% filter(år == min(år)) %>% filter(skillnad == min(.$skillnad)) %>%  .$skillnad),100),big.mark= " ")` färre förvärvsarbetande `r max(forvarvsarbetande_90_forandring$år)` jämfört med 1990. Detta beror delvis på det som diskuterades i föregående stycke, men en viktigare förklaring är sannolikt att företag inom tillverkning och utvinning idag har en mer automatiserad produktion än de hade för 30 år sedan, något som medför att det behövs färre antal anställda i produktionen.
...
<br>
```{r, echo = FALSE, message = FALSE, warning = FALSE,fig.width=10, fig.align='left', out.width="90%", fig.cap="Figur 33"}
...
<br>
```{r, echo = FALSE, message = FALSE, warning = FALSE,fig.width=10, fig.align='left', out.width="90%", fig.cap="Figur 34"}
...
till. Rent generellt ligger matchninggraden på runt `r round(matchning_df %>% filter(kön == "kvinnor",fodelseland == "totalt",region == "Sverige") %>% .$matchningsgrad,0)` procent för kvinnor och `r round(matchning_df %>% filter(kön == "män",fodelseland == "totalt",region == "Sverige") %>% .$matchningsgrad,0)` i
Sverige. Det finns vissa skillnader mellan länen, men dessa är relativt
små. I Dalarna är matchningsgraden ca
`r round(matchning_df %>% filter(kön == "kvinnor",fodelseland == "totalt",region == "Dalarna") %>% .$matchningsgrad,0)` procent för kvinnor och `r round(matchning_df %>% filter(kön == "män",fodelseland == "totalt",region == "Dalarna") %>% .$matchningsgrad,0)` , vilket alltså är något sämre än riket som helhet.

```{r, echo = FALSE, message = FALSE, warning = FALSE,fig.width=10, fig.align='left', out.width="90%", fig.cap="Figur 35"}
